...
def hhmmss_to_seconds(t: str) -> float:
    ...
    if len(parts) == 3:
        h, m, s = parts
    elif len(parts) == 2:  # sometimes VTT can be mm:ss.xxx
...
def try_youtube_transcript_api(video_id: str):
    try:
        ...
        print("youtube_transcript_api path:", youtube_transcript_api.__file__)
        from youtube_transcript_api import YouTubeTranscriptApi, TranscriptsDisabled, NoTranscriptFound

        if not hasattr(YouTubeTranscriptApi, "get_transcript"):
            raise AttributeError("Installed youtube-transcript-api has no get_transcript (bad install or wrong module).")
        ...
        return None  # None indicates a hard failure (e.g., bad import)
